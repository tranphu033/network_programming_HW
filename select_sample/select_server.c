...
int main()
{
	...
	while (1) {
		readfds = allset;		/* structure assignment; update the newest version of allset */
		nready = select(maxfd+1, &readfds, NULL, NULL, NULL); //so luong mo ta file da san sang (tinh ca listenfd)
		printf("nready=%d\n", nready);
		if(nready < 0){ 
			perror("\nError: ");
			return 0;
		...
		if (FD_ISSET(listenfd, &readfds)) {	/* new client connection - fd moi co gia tri bang listenfd */
